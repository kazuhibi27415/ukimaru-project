...
3.5 APIの取得場所と方法
	...
	https://console.cloud.google.com/welcome へアクセスする。
	
	プロジェクトの作成をする
	
	上の検索欄に「youtube」を入れると「YouTube Data API v3」があるから選択する

	「有効にする」を押す

	左側の認証情報を押す

	黄色い枠のとこの「同意画面を構成」を押す
	
	「開始」を押す

	アプリ名に適当なアプリ名を入れる・ユーザーサポートメールを設定する

	
	外部を選択する ・メールアドレスを設定する・ポリシーに同意する・完了する

	
	設定が終わったら左上の「GoogleCloud」を押す・クイックアクセスにある「APIとサービス」を押す

	一番したに「YouTube Data API v3」があるから押す・「認証情報」を押す・「認証情報を作成」を押す・「APIキー」を選択する

	名前を適当に設定・APIの制限は「YouTube Data API v3」を選択する・「完了」を押す

	APIキーをconfig.iniに入れる
	...
	https://pavlok.readme.io/reference/intro/getting-started にアクセスする・「Get API Key」を押す

	登録してあるメールアドレスを入力する・「Send Login Link」を押す・メールが送られてくるから「Log In」を押す

	Credentialsのとこに「・・・・・・・・・」とその右に鍵マークがあるので鍵マークを押す

	「Bearer ey.....」のように表示されるのでそれをすべてコピーしてconfig.iniに入れる
...
　こんな感じのURL「https://www.youtube.com/watch?v=8G8W1jrk60A」
