...
ユーザーとのUI・開始停止の相談に基づく、本文のコンソール限定制約への例外。
- 通常起動はTkinterの設定画面。`--console`で従来の監視処理を直接起動できる。
- GUIは認証、4グループ、実送信ON/OFF、delay/cooldown、履歴設定を編集できる。
- 検証規則はapp_configで共有する。保存時は外部config.iniの説明コメントを保持する。
- 保存ボタンは保存のみ。開始ボタンは検証・保存後、別プロセスで既存コンソールを実行する。
- API・金額判定・キュー・Zap処理の責務は従来のまま。GUIにAPIロジックを複製しない。
- 監視中は設定編集・保存・開始を無効化し、終了確認まで再起動できない。
- 停止・画面終了時は専用監視プロセスを終了し、未送信キューを破棄する。
- 送信済みZapは取消不可。再開時は新しい監視として初回履歴設定を適用する。
- 配信URLは永続化しない。ログは画面内のみ、最大約2000行。認証欄は伏せ字。
- PyInstallerはconsole=Falseとし、GUI・監視用子プロセスのコマンド画面を表示しない。
- GUIの出力方法は「固定」「ランダム」と表示し、config.iniではfixed/randomを維持する。
- --gui-workerでは継承パイプをUTF-8で開き直す。明示的な--console起動のみコンソールへ接続する。
